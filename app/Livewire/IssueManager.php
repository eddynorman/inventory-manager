...
class IssueManager extends Component
{
    ...
    public function updatedSearch()
    {
        ...
        $this->searchItems = $this->service->search(
            $this->destinationId,
            $this->search
        );
    }
    ...
    public function updatedIssueItems($value, $keys)
    {
        ...
        */
        ...
        if (!isset($this->issueItems[$index])) {
            return;
        }

        $item = $this->issueItems[$index];
        ...
        */
        ...
        $unit = collect($item['units'])
            ->firstWhere('id', $item['selected_unit_id']);
        ...
        */
        ...
        if ($key === 'quantity' || $key === 'selected_unit_id') {
            ...
            if ($q > $item['stock']) {
                ...
                $this->addError(
                    "issueItems.$index.quantity",
                    "Cannot exceed stock"
                );
                $this->dispatch(
                    'issue-errors-updated',
